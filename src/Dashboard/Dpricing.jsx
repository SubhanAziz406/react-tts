  ...
  const handleGetStarted = async (priceId) => {
    ...
    if (!userId) {
      toast.warn("Please log in to subscribe.", {
        position: "top-center",
        autoClose: 3000,
        onClose: () => navigate("/login"), // Redirect to login if not logged in
      });
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${CONFIG.baseURL}/api/create-payment-intent`,
        {
          userID: localStorage.getItem('userId'),
          priceId: 'price_1QU80NH4JsYIDHElMgqyb1kH',
          successUrl: `${CONFIG.baseURL}/successpage`,
          cancelUrl:  `${CONFIG.frontendBaseUrl}/cancel`, // Pass the priceId of the selected plan
        }
      );
